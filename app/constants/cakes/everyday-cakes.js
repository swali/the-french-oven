import assetUrl from 'the-french-oven/utils/asset-url';

export default [
  {
    name: 'Black Forest Cake',
    description:
      'Two chocolate sponges with a cherry pastry filling, finished with non-dairy whipped topping and decorated with rosettes, candied cherries and chocolate shavings.',
    imageUrl: assetUrl('/assets/images/cakes/7_inch_black_forest.jpg'),
  },
  {
    name: 'Carrot Cake',
    description:
      'Two rounds of carrot cake filled with cream cheese icing, finished with cream cheese icing and decorated with icing carrots.',
    imageUrl: assetUrl('/assets/images/cakes/carrot.jpg'),
  },
  {
    name: 'Chocolate Caramel Cake',
    description:
      'Moist chocolate cake with rich melt-in-your-mouth caramel, decorated with chocolate shavings and finished with an elegant chocolate garnish.',
    imageUrl: assetUrl('/assets/images/cakes/7_inch_caramel.jpg'),
  },
  {
    name: 'Kit Kat With Fudge Cake',
    description:
      'Two layers of chocolate cake with a fudge filling, decorated with lots of Kit Kat pieces.',
    imageUrl: assetUrl('/assets/images/cakes/7-inch-kitkatfudge.jpg'),
  },
  {
    name: 'Cookies and Cream Cake',
    description:
      'Two layers of chocolate cake filled with chocolate cookie crumbs, decorated with cookie crunch and two oreo cookies.',
    imageUrl: assetUrl('/assets/images/cakes/cookies-and-cream.jpg'),
  },
  {
    name: 'Mocha Cake',
    description:
      'Two layers of vanilla cake with mocha crème and caramel filling decorated with glazed coffee on top and side finished with drizzle of caramel and chocolate decoration on top.',
    imageUrl: assetUrl('/assets/images/cakes/mocha.jpg'),
  },
  {
    name: 'Mocha Caramel Cake',
    description:
      'Two layers of vanilla sponge with mocha crème filling finished with a layer of caramel filling on the top and side decorated with chocolate curls on the side and chocolate drizzle on top.',
    imageUrl: assetUrl('/assets/images/cakes/mocha-caramel.jpg'),
  },
  {
    name: 'Lemon Cake',
    description:
      'Two layers of vanilla cake with lemon crème filling finished with a layer of lemon filling on the top and side decorated with chocolate curls on side and chocolate drizzle on top.',
    imageUrl: assetUrl('/assets/images/cakes/lemon.jpg'),
  },
  {
    name: 'Double Chocolate Cake',
    description:
      'Two layers of chocolate cake with chocolate crème filling finished with a layer of chocolate ganache on the top and side of cake decorated with chocolate curls on the side and white chocolate drizzle on top.',
    imageUrl: assetUrl('/assets/images/cakes/double-chocolate.jpg'),
  },
  {
    name: 'Pineapple Coconut Cake',
    description:
      'Two layers of vanilla cake with pineapple crème and coconut filling decorated with pineapple filling on top and side of cake finished with shredded coconut on side of cake and a drizzle of chocolate on top.',
    imageUrl: assetUrl('/assets/images/cakes/pineapple-coconut.jpg'),
  },
  {
    name: 'Strawberry Cake',
    description:
      'Two layers of vanilla cake with strawberry crème filling decorated with a layer of strawberry filling on top and side, finished with chocolate curls on side.',
    imageUrl: assetUrl('/assets/images/cakes/strawberry.jpg'),
  },
];
